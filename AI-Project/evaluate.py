...
import sample_agent 
import your_agent  
from your_agent import apply_move,check_win
def play_game(agent_x, agent_o, board_size=3) -> float:
    ...
    while True:
        ...
        print(f'the opponent is : {agent_o}')
        ...
        if symbol == "O":
            ...
            symbol_player = "X"
            symbol_opponent = "O"
        winner = check_win(board, symbol_player)
        winnerـo = check_win(board, symbol_opponent)
        
        if winner:return 1.0
        elif winnerـo: return 0.0
...
if __name__ == "__main__":
    win_pct = evaluate_agent(your_agent.agent_move, sample_agent.agent_move, games=30,board_size=3)
